...
export default function AnalyticsPage() {
  ...
  const pending = totalApplications - screening - interviews - offers - rejections;

  // Cover letter impact
  const clScreeningRate = withCoverLetter > 0 
    ? ((applications.filter((app) => app.DidCL && app.DateScreening).length / withCoverLetter) * 100).toFixed(1)
    : "0";
  const noclScreeningRate = withoutCoverLetter > 0
    ? ((applications.filter((app) => !app.DidCL && app.DateScreening).length / withoutCoverLetter) * 100).toFixed(1)
    : "0";

  const clInterviewRate = withCoverLetter > 0
    ? ((applications.filter((app) => app.DidCL && app.DateInterview).length / withCoverLetter) * 100).toFixed(1)
    : "0";
  const noclInterviewRate = withoutCoverLetter > 0
    ? ((applications.filter((app) => !app.DidCL && app.DateInterview).length / withoutCoverLetter) * 100).toFixed(1)
    : "0";

  const clOfferRate = withCoverLetter > 0
    ? ((applications.filter((app) => app.DidCL && app.DateAccepted).length / withCoverLetter) * 100).toFixed(1)
    : "0";
  const noclOfferRate = withoutCoverLetter > 0
    ? ((applications.filter((app) => !app.DidCL && app.DateAccepted).length / withoutCoverLetter) * 100).toFixed(1)
    : "0";
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      ...
      {usingSampleData && (
        <div className="mb-6 p-4 bg-gradient-to-r from-[#3CA2C8]/10 to-[#10559A]/10 border border-[#3CA2C8] rounded-lg">
          <div className="flex items-start">
            <svg className="w-5 h-5 text-[#10559A] mr-3 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
              <h3 className="font-semibold text-[#10559A] mb-1">Using Sample Data</h3>
              <p className="text-sm text-gray-600">
                No application data found in DynamoDB. Showing sample analytics to demonstrate features. 
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Data-driven insights about your job search</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <Card className="border-0 shadow-lg">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-[#10559A]">{totalApplications}</div>
          </CardContent>
        </Card>

        <Card className="border-0 shadow-lg">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-600">Interview Rate</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-[#3CA2C8]">
              {totalApplications > 0 ? ((interviews / totalApplications) * 100).toFixed(1) : 0}%
            </div>
          </CardContent>
        </Card>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm text-gray-600">{pending} ({totalApplications > 0 ? ((pending / totalApplications) * 100).toFixed(0) : 0}%)</span>
              </div>
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm text-gray-600">{screening} ({totalApplications > 0 ? ((screening / totalApplications) * 100).toFixed(0) : 0}%)</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-3">
                <div
                  className="bg-[#3CA2C8] h-3 rounded-full"
                  style={{ width: `${totalApplications > 0 ? (screening / totalApplications) * 100 : 0}%` }}
                ></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm text-gray-600">{interviews} ({totalApplications > 0 ? ((interviews / totalApplications) * 100).toFixed(0) : 0}%)</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-3">
                <div
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm text-gray-600">{offers} ({totalApplications > 0 ? ((offers / totalApplications) * 100).toFixed(0) : 0}%)</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-3">
                <div
                  className="bg-green-500 h-3 rounded-full"
                  style={{ width: `${totalApplications > 0 ? (offers / totalApplications) * 100 : 0}%` }}
                ></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm text-gray-600">{rejections} ({totalApplications > 0 ? ((rejections / totalApplications) * 100).toFixed(0) : 0}%)</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-3">
                <div
                  className="bg-red-500 h-3 rounded-full"
                  style={{ width: `${totalApplications > 0 ? (rejections / totalApplications) * 100 : 0}%` }}
                ></div>
              </div>
            </div>
          </div>
                ...
                <div className="flex gap-4">
                  <div className="flex-1">
                    <div className="text-center mb-2">
                      <div className="text-2xl font-bold text-[#10559A]">{withCoverLetter}</div>
                      <div className="text-xs text-gray-500">With CL</div>
                    </div>
                  </div>
                  <div className="flex-1">
                    <div className="text-center mb-2">
                      <div className="text-2xl font-bold text-gray-400">{withoutCoverLetter}</div>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm font-medium">Screening Rate</span>
                    </div>
                    <div className="flex gap-4 items-end">
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-center text-gray-500 mt-1">With CL</div>
                      </div>
                      <div className="flex-1">
                        <div className="bg-gray-300 text-gray-700 text-center py-2 rounded-lg font-semibold">
                          {noclScreeningRate}%
                        </div>
                        <div className="text-xs text-center text-gray-500 mt-1">Without CL</div>
                      </div>
                    </div>
                  </div>
              ...
              {withCoverLetter > 0 && withoutCoverLetter > 0 && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm text-blue-900">
                    <strong>Insight:</strong> Applications with cover letters have a{" "}
                    {parseFloat(clScreeningRate) > parseFloat(noclScreeningRate) ? "higher" : "lower"} screening rate
                    ({Math.abs(parseFloat(clScreeningRate) - parseFloat(noclScreeningRate)).toFixed(1)}% difference).
                  </p>
                </div>
              )}
            </div>
                ...
                topCompanies.map(([company, count]) => (
                  <div key={company}>
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-sm text-gray-600">{count} {count === 1 ? 'application' : 'applications'}</span>
                    </div>
            ...
            <div className="space-y-6">
              <div className="text-center">
                <div className="text-5xl font-bold text-[#10559A] mb-2">
                  {avgDaysToScreening > 0 ? Math.round(avgDaysToScreening) : "—"}
                </div>
              ...
              <div className="border-t pt-6 text-center">
                <div className="text-5xl font-bold text-[#3CA2C8] mb-2">
                  {avgDaysToInterview > 0 ? Math.round(avgDaysToInterview) : "—"}
                </div>
          ...
          <CardContent>
            <div className="space-y-6">
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-2xl font-bold text-[#3CA2C8]">
                    {totalApplications > 0 ? ((screening / totalApplications) * 100).toFixed(0) : 0}%
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-2xl font-bold text-green-600">
                    {interviews > 0 ? ((offers / interviews) * 100).toFixed(0) : 0}%
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className="bg-green-500 h-2 rounded-full"
                    style={{ width: `${interviews > 0 ? (offers / interviews) * 100 : 0}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-[#DB4C77] to-[#F9C6D7] rounded-lg p-4 text-white mt-4">
                ...
                <p className="text-xs opacity-90 mt-1">of applications result in offers</p>
              </div>
            </div>
          </CardContent>
        </Card>
