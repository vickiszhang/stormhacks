...
export default function Home() {
  ...
  const getStatusDate = (statusHistory: { status: status; date: Date }[], targetStatus: status) => {
        ...
        <Table>
          <TableBody>
            {sampleApplicationData.map((application, index) => {
              const appliedDate = getStatusDate(application.statusHistory, status.APPLIED);
              const screenDate = getStatusDate(application.statusHistory, status.ONLINE_ASSESSMENT);
              const interviewDate = getStatusDate(application.statusHistory, status.INTERVIEW);
              const offerDate = getStatusDate(application.statusHistory, status.OFFER);
              const rejectedDate = getStatusDate(application.statusHistory, status.REJECTED);

              return (
                <TableRow key={index} className={`${rejectedDate ? "bg-gray-100 text-muted-foreground" : ""} h-16`}>
                  <TableCell>
